...
import { loadCovers, submitCover } from './covers.js';
import { initAuth, onAuthChange, getUser, getProfile, signInWithGoogle, signInWithSpotify, signOut, isCurator } from './auth.js';
...
function updateMeta(cover) {
  ...
  if (cover.submittedBy) {
    vinylBtn.textContent = 'Check vinyl availability';
    vinylBtn.classList.add('vinyl-hint');
  } else {
    vinylBtn.textContent = 'Buy Vinyl on Amazon';
...
function updateSaveButton(cover) {
  ...
  const saved = isSaved(cover.id);
  saveBtn.textContent = saved ? 'Saved' : 'Save to Collection';
  ...
  onAuthChange((user, profile) => {
    if (user && profile) {
      ...
      if (profile.avatar_url) {
        ...
        avatarImg.alt = profile.display_name || 'User';
      } else {
        avatarImg.src = '';
        avatarImg.alt = profile.display_name?.[0] || 'U';
      }
...
function initSubmitModal() {
  ...
  document.getElementById('submitToGithub')?.addEventListener('click', async () => {
    ...
    const genreRaw = document.getElementById('genreInput').value.trim();
